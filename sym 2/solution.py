...
def func1(file_in : str) -> list[str]:
    ...
    #we create a list of words after clearing them
    words = []
    current = ""
...
def func2(file_in_a: str, file_in_b: str) -> list[str]:
    ...
    with open(file_in_a, "r") as f:
        chars_a = set(c for c in f.read() if c not in {'\n', '\t', ' '})
    
    with open(file_in_b, "r") as f:
        chars_b = set(c for c in f.read() if c not in {'\n', '\t', ' '})
    
    only_a = sorted(chars_a - chars_b)
    
    only_b = sorted(chars_b - chars_a)
...
def func3(lists : list[list[str]], listi : list[list[int]], out : str) -> int:
    ...
    with open(out, "w") as f:
        for words, indices in zip(lists, listi):

            ordered_words = [words[i] for i in indices]
...
def func4(input_file : str, output_file : str) -> int:
    ...
    # Order words for length, case insenstive, case sensitive. We need both w.lower() and w because using only w 
    #would put B before a which is incorrect
    words_sorted = sorted(words, key=lambda w: (len(w), w.lower(), w))

    with open(output_file, "w") as f:
        f.write(" ".join(words_sorted))

    return len(words_sorted)
...
def func5(input_filename : str, output_filename : str, length : int) -> int:
    ...
    with open(input_filename, "r") as f:
        for line in f:
            words.extend(line.split()) #add all the elements of line.split to the end of words

    # list comprehension to get only the wanted length words
    words = [w for w in words if len(w) == length]

    # group words for 1st letter case insensitive
    groups = {}
    for w in words:
        key = w[0].lower()
        groups.setdefault(key, []).append(w)

    # Orders keys case sensitive
    for key in groups:
        groups[key].sort(key=lambda w: (w.lower(), w))

    # writes on the file on seperate lines
    with open(output_filename, "w", encoding="utf-8") as f:
        for key in sorted(groups.keys()):
            f.write(" ".join(groups[key]) + "\n")
